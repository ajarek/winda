    ...
    element.addEventListener('click', () => {
        ...
        for(i=0;i<tablica.length;i++){
            if(tablica[tablica.length-1]>tablica[tablica.length-2]){
                ...
                animateUp(display, parseInt(tablica[tablica.length-2]), parseInt(tablica[tablica.length-1]), 6000);
            }
            if(tablica[tablica.length-1]<tablica[tablica.length-2]){
              
                animateDown(display, parseInt(tablica[tablica.length-2]), parseInt(tablica[tablica.length-1]), 6000);
            }
        }
    })
